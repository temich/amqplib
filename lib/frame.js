...
/**
 * The length of the frame the bytes begin, header and end included, or 0 while its header has
 * not arrived.
 * @param { Buffer } bin
 * @returns { number }
 */
module.exports.frameLength = (bin) => {
  if (bin.length < FRAME_HEADER_BYTES) return 0;
  return FRAME_HEADER_BYTES + bin.readUInt32BE(3) + FRAME_END_BYTES;
};
