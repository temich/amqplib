...
// A body frame's content is either a buffer, or the buffers it arrived
// in when it spanned more than one read from the socket
function gather(buffers, content) {
  if (Array.isArray(content)) for (const piece of content) buffers.push(piece);
  else buffers.push(content);
}
...
function acceptMessage(continuation) {
  ...
  function content(f) {
    if (f.content) {
      ...
      if (remaining === 0) {
        if (buffers !== null) {
          gather(buffers, f.content);
          message.content = Buffer.concat(buffers);
        } else if (Array.isArray(f.content)) {
          message.content = Buffer.concat(f.content, size);
        } else {
