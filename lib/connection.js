...
class Connection extends EventEmitter {
  constructor(underlying) {
    ...
    // the chunks of a frame whose header has arrived and whose end has not; see `gather`
    this.pending = null;
  ...
  */
  ...
  recvFrame() {
    for (;;) {
      ...
      this.gather(incoming);
    }
  }

  // Adds a chunk to what has arrived. Once a frame's header is in, its length is known, and its
  // chunks are held in a list and joined once, when the last of them arrives: joining on every
  // chunk would copy what is already there again each time, which for a frame of `frameMax`
  // arriving in socket-sized chunks is many times the frame.
  gather(incoming) {
    if (this.pending !== null) {
      this.pending.push(incoming);
      this.pendingLength += incoming.length;
    } else if (frameLength(this.rest) !== 0) {
      this.pending = [this.rest, incoming];
      this.pendingLength = this.rest.length + incoming.length;
      this.pendingFrameLength = frameLength(this.rest);
    } else {
      this.rest = this.rest.length === 0 ? incoming : Buffer.concat([this.rest, incoming]);

      const length = frameLength(this.rest);
      if (length === 0 || this.rest.length >= length) return;

      this.pending = [this.rest];
      this.pendingLength = this.rest.length;
      this.pendingFrameLength = length;
    }

    if (this.pendingLength < this.pendingFrameLength) return;

    this.rest = Buffer.concat(this.pending, this.pendingLength);
    this.pending = null;
  }
}
