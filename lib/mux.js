...
class Mux {
  ...
  _readIncoming() {
    ...
    // Frames are written one at a time, and a request served is two of them: the reply, and the
    // acknowledgement of the request it answers. Over a socket that sends without delay each is a
    // syscall and a packet of its own. Corked for the round, they leave together, in one write,
    // and nothing waits for a timer to send them.
    const corked = typeof out.cork === 'function';

    if (corked) out.cork();

    // uncorked however the round ends: an assertion below that throws would otherwise leave the
    // socket holding every frame written after it
    try {
      this.#round(out);
    } finally {
      if (corked) out.uncork();
    }
  }
